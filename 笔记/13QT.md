...
### **5. λ表达式信号与槽**
        ...
        connect(this, &MainWindow::signalA, [](int i){
            qDebug() << "λ表达式输出" << i;
        });
### **6. connect的传统写法和新式写法**
1. 传统写法
    - `connect(this, SIGNAL(signalA(int)), this, SLOT(slotA(int)));`
2. 新式写法
    - `connect(this, &MainWindow::signalA, this, &MainWindow::slotA);`
    - **若槽函数有重载, 你想要使用你想要的槽函数时怎么办?**
        - `static cast对函数进行强转`
            - `connect(this, &MainWindow::signalA, this, static_cast<void (MainWindow::*)(int)>(&MainWindow::slotA));`
            - [**参考C++强制类型转换**](./12C%2B%2B.md##C++强制类型转换)
...
3. [元对象编译器(moc)**只又类直接或间接继承QOBJECT时才会触发**](../../../resource/2.QT/3.moc%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86.png)
    ...
    2. 有的话再找Qt关键字
    3. 以上获取到的内容称为**元数据**
    4. moc会把元数据编译出来, 生成`moc_*.cpp`文件
    5. 在编译阶段把其和源文件组合起来
4. 总结: `moc`就是把编译器看不懂的关键字进行元对象编译
---
## ***QT内存回收机制***
...
5. **将qss写入文件中, 用的时候通过函数调用**

        QFile qss(":/qss/dark.qss");    // 所有的样式写入.qss中
